"""Shared block parser for the diff/patch revision mode toolchain (ARS Spec #390).

Normative source: ARS Spec #390 §3.1 (block segmentation, marker grammar,
malformed-state rules, unsupported-construct rejection).
Both `ars_anchorize_draft.py` and `ars_apply_revision_patch.py` import this module
so segmentation can never drift between the stamping side and the splicing side.

Design constraints the implementation must not violate:
  - Fail-closed, never guess. Anything the line-based scan cannot classify into
    the §3.1 supported block classes raises BlockParseError naming the construct.
  - Byte-span fidelity. Blocks carry character offsets into the original text
    (UTF-8 decoded, no newline translation) so the apply script can splice the
    original byte stream. The parser never re-serializes content.
  - Hash normalization is read-side only (§3.2): CRLF->LF, marker line excluded,
    block-level leading/trailing blank lines stripped, intra-line whitespace
    untouched. Normalized text exists for hash computation and is never written back.

Supported block classes (§3.1, closed list): fenced code, ATX heading, table run,
list run, blockquote run, plain text run, plus skipped YAML frontmatter.
Setext-underline shapes, line-initial raw-HTML openers (^</?[A-Za-z]), and footnote
definitions are rejected by name.
"""
...
class BlockParseError(ValueError):
    ...
    `kind` is a stable machine-readable failure class; `line_no` is 1-based;
    `hint` is the human repair hint for orphan markers and friends.
    """
...
class Block:
    ...
    span: tuple[int, int]  # char span of content lines incl. trailing EOL (if present)
...
def split_lines_keepends(text: str) -> list[str]:
    ...
    desynchronize char offsets from markdown line semantics.
    """
    lines = text.split("\n")
    out = [line + "\n" for line in lines[:-1]]
    if lines[-1] != "":
...
def segment_fragment(new_text: str) -> list[Block]:
    ...
    Returns parsed Block objects (spans into new_text). Raises BlockParseError
    on unsupported constructs, embedded markers, or empty fragments.
    """
