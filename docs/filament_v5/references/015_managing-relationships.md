...
### [​](#relation-managers-interactive-tables-underneath-your-resource-forms)Relation managers - interactive tables underneath your resource forms
...
### [​](#select-&-checkbox-list-choose-from-existing-records-or-create-a-new-one)Select & checkbox list - choose from existing records or create a new one

These are compatible with `BelongsTo`, `MorphTo` and `BelongsToMany` relationships.

Using a [select](/docs/5.x/forms/select#integrating-with-an-eloquent-relationship), users will be able to choose from a list of existing records. You may also [add a button that allows you to create a new record inside a modal](/docs/5.x/forms/select#creating-new-records), without leaving the page. When using a `BelongsToMany` relationship with a select, you’ll be able to select multiple options, not just one. Records will be automatically added to your pivot table when you submit the form. If you wish, you can swap out the multi-select dropdown with a simple [checkbox list](/docs/5.x/forms/checkbox-list#integrating-with-an-eloquent-relationship). Both components work in the same way.

### [​](#repeaters-crud-multiple-related-records-inside-the-owner’s-form)Repeaters - CRUD multiple related records inside the owner’s form

These are compatible with `HasMany` and `MorphMany` relationships.

[Repeaters](/docs/5.x/forms/repeater#integrating-with-an-eloquent-relationship) are standard form components, which can render a repeatable set of fields infinitely. They can be hooked up to a relationship, so records are automatically read, created, updated, and deleted from the related table. They live inside the main form schema, and can be used inside resource pages, as well as nesting within action modals. From a UX perspective, this solution is only suitable if your related model only has a few fields. Otherwise, the form can get very long.

### [​](#layout-form-components-saving-form-fields-to-a-single-relationship)Layout form components - saving form fields to a single relationship

These are compatible with `BelongsTo`, `HasOne` and `MorphOne` relationships.

All layout form components ([Grid](/docs/5.x/schemas/layouts#grid-component), [Section](/docs/5.x/schemas/sections), [Fieldset](/docs/5.x/schemas/layouts#fieldset-component), etc.) have a [`relationship()` method](/docs/5.x/forms/overview#saving-data-to-relationships). When you use this, all fields within that layout are saved to the related model instead of the owner’s model:
...
- `title` is the name of the attribute that will be used to identify posts.This will create a `CategoryResource/RelationManagers/PostsRelationManager.php` file. This contains a class where you are able to define a [form](/docs/5.x/resources/overview#resource-forms) and [table](/docs/5.x/resources/overview#resource-tables) for your relation manager:
...
Once a table and form have been defined for the relation manager, visit the [Edit](/docs/5.x/resources/editing-records) or [View](/docs/5.x/resources/viewing-records) page of your resource to see it in action.

### [​](#customizing-the-relation-manager’s-url-parameter)Customizing the relation manager’s URL parameter
...
Alternatively, if you hate this functionality, you can disable it for all relation managers at once in the panel [configuration](/docs/5.x/panel-configuration):
...
### [​](#unconventional-inverse-relationship-names)Unconventional inverse relationship names
...
### [​](#creating-with-pivot-attributes)Creating with pivot attributes
...
### [​](#customizing-the-createaction)Customizing the `CreateAction`

To learn how to customize the `CreateAction`, including mutating the form data, changing the notification, and adding lifecycle hooks, please see the [Actions documentation](/docs/5.x/actions/create).

## [​](#editing-related-records)Editing related records

### [​](#editing-with-pivot-attributes)Editing with pivot attributes
...
### [​](#customizing-the-editaction)Customizing the `EditAction`

To learn how to customize the `EditAction`, including mutating the form data, changing the notification, and adding lifecycle hooks, please see the [Actions documentation](/docs/5.x/actions/edit).

## [​](#attaching-and-detaching-records)Attaching and detaching records
...
`AssociateAction`, `AttachAction`, `DetachAction`, and `DissociateAction` (and their bulk variants) only check the relation manager’s `isReadOnly()` state — they do not consult any model policy method by default. Bulk delete, force-delete, and restore actions use the `deleteAny()`, `forceDeleteAny()`, and `restoreAny()` policy methods (one call for the whole batch) for performance. If you need per-record authorization on a bulk action, call [`authorizeIndividualRecords('ability')`](/docs/5.x/actions/overview#authorizing-individual-records-of-a-bulk-action) on it, accepting the extra query cost.

### [​](#preloading-the-attachment-modal-select-options)Preloading the attachment modal select options
...
### [​](#attaching-with-pivot-attributes)Attaching with pivot attributes
...
In this example, `$action->getRecordSelect()` returns the select field to pick the record to attach. The `role` text input is then saved to the pivot table’s `role` column. Please ensure that any pivot attributes are listed in the `withPivot()` method of the relationship *and* inverse relationship.

### [​](#scoping-the-options-to-attach)Scoping the options to attach
...
### [​](#searching-the-options-to-attach-across-multiple-columns)Searching the options to attach across multiple columns
...
### [​](#attaching-multiple-records)Attaching multiple records
...
### [​](#customizing-the-select-field-in-the-attached-modal)Customizing the select field in the attached modal
...
#### [​](#selecting-records-to-attach-using-a-modal-table)Selecting records to attach using a modal table
...
Filtering the table’s query (via `modifyQueryUsing()`, filters, or table arguments) is **presentational** — it only affects which records are displayed for selection. It is **not** a security boundary: a user who tampers with the submitted modal state can attach a record that was excluded from the visible table.To restrict which records may actually be attached, scope the options using the [`recordSelectOptionsQuery()` method](#scoping-the-options-to-attach). Filament resolves the submitted record against that query, so records outside it are rejected:
...
By default, you will not be allowed to attach a record more than once. This is because you must also set up a primary `id` column on the pivot table for this feature to work. Please ensure that the `id` attribute is listed in the `withPivot()` method of the relationship *and* inverse relationship. Finally, add the `allowDuplicates()` method to the table:
...
### [​](#improving-the-performance-of-detach-bulk-actions)Improving the performance of detach bulk actions

By default, the `DetachBulkAction` will load all Eloquent records into memory, before looping over them and detaching them one by one. If you are detaching a large number of records, you may want to use the `chunkSelectedRecords()` method to fetch a smaller number of records at a time. This will reduce the memory usage of your application:
...
## [​](#associating-and-dissociating-records)Associating and dissociating records
...
### [​](#preloading-the-associate-modal-select-options)Preloading the associate modal select options
...
### [​](#scoping-the-options-to-associate)Scoping the options to associate
...
### [​](#searching-the-options-to-associate-across-multiple-columns)Searching the options to associate across multiple columns
...
### [​](#associating-multiple-records)Associating multiple records
...
### [​](#customizing-the-select-field-in-the-associate-modal)Customizing the select field in the associate modal
...
## [​](#viewing-related-records)Viewing related records
...
### [​](#customizing-the-deleteaction)Customizing the `DeleteAction`

To learn how to customize the `DeleteAction`, including changing the notification and adding lifecycle hooks, please see the [Actions documentation](/docs/5.x/actions/delete).

## [​](#importing-related-records)Importing related records

The [`ImportAction`](/docs/5.x/actions/import) can be added to the header of a relation manager to import records. In this case, you probably want to tell the importer which owner these new records belong to. You can use [import options](/docs/5.x/actions/import#using-import-options) to pass through the ID of the owner record:
...
## [​](#accessing-the-relationship’s-owner-record)Accessing the relationship’s owner record
...
However, if you’re inside a `static` method like `form()` or `table()`, `$this` isn’t accessible. So, you may [use a callback](/docs/5.x/forms/overview#field-utility-injection) to access the `$livewire` instance:
...
## [​](#grouping-relation-managers)Grouping relation managers
...
## [​](#conditionally-showing-relation-managers)Conditionally showing relation managers

By default, relation managers will be visible if the `viewAny()` method for the related model policy returns `true`. You may use the `canViewForRecord()` method to determine if the relation manager should be visible for a specific owner record and page:
...
## [​](#combining-the-relation-manager-tabs-with-the-form)Combining the relation manager tabs with the form
...
### [​](#customizing-the-content-tab)Customizing the content tab

On the Edit or View page class, override the `getContentTabComponent()` method, and use any [Tab](/docs/5.x/schemas/tabs) customization methods:
...
### [​](#setting-the-position-of-the-form-tab)Setting the position of the form tab
...
## [​](#customizing-relation-manager-tabs)Customizing relation manager tabs

To customize the tab for a relation manager, override the `getTabComponent()` method, and use any [Tab](/docs/5.x/schemas/tabs) customization methods:
...
### [​](#deferring-the-loading-of-relation-manager-tab-badges)Deferring the loading of relation manager tab badges
...
## [​](#sharing-a-resource’s-form-and-table-with-a-relation-manager)Sharing a resource’s form and table with a relation manager
...
### [​](#hiding-a-shared-form-component-on-the-relation-manager)Hiding a shared form component on the relation manager
...
### [​](#hiding-a-shared-table-column-on-the-relation-manager)Hiding a shared table column on the relation manager
...
### [​](#hiding-a-shared-table-filter-on-the-relation-manager)Hiding a shared table filter on the relation manager
...
### [​](#overriding-shared-configuration-on-the-relation-manager)Overriding shared configuration on the relation manager
...
## [​](#customizing-the-relation-manager-eloquent-query)Customizing the relation manager Eloquent query
...
## [​](#customizing-the-relation-manager-title)Customizing the relation manager title
...
The title will be reflected in the [heading of the table](/docs/5.x/tables/overview#customizing-the-table-header), as well as the relation manager tab if there is more than one. If you want to customize the table heading independently, you can still use the `$table->heading()` method:
...
## [​](#customizing-the-relation-manager-record-title)Customizing the relation manager record title
...
In this example, the `title` attribute of the `Post` model will be used to identify a post in the relation manager. This is mainly used by the action classes. For instance, when you [attach](#attaching-and-detaching-records) or [associate](#associating-and-dissociating-records) a record, the titles will be listed in the select field. When you [edit](#editing-related-records), [view](#viewing-related-records) or [delete](#deleting-related-records) a record, the title will be used in the header of the modal. In some cases, you may want to concatenate multiple attributes together to form a title. You can do this by replacing the `recordTitleAttribute()` configuration method with `recordTitle()`, passing a function that transforms a model into a title:
...
## [​](#relation-pages)Relation pages

Using a `ManageRelatedRecords` page is an alternative to using a relation manager, if you want to keep the functionality of managing a relationship separate from editing or viewing the owner record. This feature is ideal if you are using [resource sub-navigation](/docs/5.x/resources/overview#resource-sub-navigation), as you are easily able to switch between the View or Edit page and the relation page. To create a relation page, you should use the `make:filament-page` command:

```
php artisan make:filament-page ManageCustomerAddresses --resource=CustomerResource --type=ManageRelatedRecords
```

When you run this command, you will be asked a series of questions to customize the page, for example, the name of the relationship and its title attribute. You must register this new page in your resource’s `getPages()` method:
...
### [​](#adding-relation-pages-to-resource-sub-navigation)Adding relation pages to resource sub-navigation

If you’re using [resource sub-navigation](/docs/5.x/resources/overview#resource-sub-navigation), you can register this page as normal in `getRecordSubNavigation()` of the resource:
...
## [​](#passing-properties-to-relation-managers)Passing properties to relation managers
...
[Deleting records](/docs/5.x/resources/deleting-records)[Nested resources](/docs/5.x/resources/nesting)
