...
## [​](#registering-render-hooks)Registering render hooks
...
## [​](#available-render-hooks)Available render hooks

### [​](#panel-builder-render-hooks)Panel Builder render hooks
...
- `PanelsRenderHook::GLOBAL_SEARCH_AFTER` - After the [global search](/docs/5.x/resources/global-search) container, inside the topbar
- `PanelsRenderHook::GLOBAL_SEARCH_BEFORE` - Before the [global search](/docs/5.x/resources/global-search) container, inside the topbar
- `PanelsRenderHook::GLOBAL_SEARCH_END` - The end of the [global search](/docs/5.x/resources/global-search) container
- `PanelsRenderHook::GLOBAL_SEARCH_START` - The start of the [global search](/docs/5.x/resources/global-search) container
...
- `PanelsRenderHook::SIDEBAR_NAV_END` - In the [sidebar](/docs/5.x/navigation), before `</nav>`
- `PanelsRenderHook::SIDEBAR_NAV_START` - In the [sidebar](/docs/5.x/navigation), after `<nav>`
...
- `PanelsRenderHook::TENANT_MENU_AFTER` - After the [tenant menu](/docs/5.x/users/tenancy#customizing-the-tenant-menu)
- `PanelsRenderHook::TENANT_MENU_BEFORE` - Before the [tenant menu](/docs/5.x/users/tenancy#customizing-the-tenant-menu)
...
- `PanelsRenderHook::USER_MENU_AFTER` - After the [user menu](/docs/5.x/navigation/user-menu)
- `PanelsRenderHook::USER_MENU_BEFORE` - Before the [user menu](/docs/5.x/navigation/user-menu)
- `PanelsRenderHook::USER_MENU_PROFILE_AFTER` - After the profile item in the [user menu](/docs/5.x/navigation/user-menu)
- `PanelsRenderHook::USER_MENU_PROFILE_BEFORE` - Before the profile item in the [user menu](/docs/5.x/navigation/user-menu)

### [​](#table-builder-render-hooks)Table Builder render hooks
...
- `TablesRenderHook::TOOLBAR_GROUPING_SELECTOR_AFTER` - After the [grouping](/docs/5.x/tables/grouping) selector
- `TablesRenderHook::TOOLBAR_GROUPING_SELECTOR_BEFORE` - Before the [grouping](/docs/5.x/tables/grouping) selector
- `TablesRenderHook::TOOLBAR_REORDER_TRIGGER_AFTER` - After the [reorder](/docs/5.x/tables/overview#reordering-records) trigger
- `TablesRenderHook::TOOLBAR_REORDER_TRIGGER_BEFORE` - Before the [reorder](/docs/5.x/tables/overview#reordering-records) trigger
- `TablesRenderHook::TOOLBAR_SEARCH_AFTER` - After the [search](/docs/5.x/tables/overview#making-columns-sortable-and-searchable) container
- `TablesRenderHook::TOOLBAR_SEARCH_BEFORE` - Before the [search](/docs/5.x/tables/overview#making-columns-sortable-and-searchable) container
- `TablesRenderHook::TOOLBAR_START` - The start of the toolbar
- `TablesRenderHook::TOOLBAR_COLUMN_MANAGER_TRIGGER_AFTER` - After the [column manager](/docs/5.x/tables/columns/overview#toggling-column-visibility) trigger
- `TablesRenderHook::TOOLBAR_COLUMN_MANAGER_TRIGGER_BEFORE` - Before the [column manager](/docs/5.x/tables/columns/overview#toggling-column-visibility) trigger

### [​](#actions-render-hooks)Actions render hooks

All these render hooks [can be scoped](#scoping-render-hooks) to any Livewire component class. When using the Panel Builder, these classes might be the List or Manage page of a resource, or a relation manager. Scoping is typically not enough in this case, as Livewire components can have multiple actions, so you can access the `action` data as `Filament\Actions\Action` to identify the specific action in all these render hooks.

```
use Filament\Actions\View\ActionsRenderHook;
```

- `ActionsRenderHook::MODAL_CUSTOM_CONTENT_AFTER` - After the [modal content](/docs/5.x/actions/modals#custom-modal-content)
- `ActionsRenderHook::MODAL_CUSTOM_CONTENT_BEFORE` - Before the [modal content](/docs/5.x/actions/modals#custom-modal-content)
- `ActionsRenderHook::MODAL_CUSTOM_CONTENT_FOOTER_AFTER` - After the [modal content footer](/docs/5.x/actions/modals#adding-custom-modal-content-below-the-form)
- `ActionsRenderHook::MODAL_CUSTOM_CONTENT_FOOTER_BEFORE` - Before the [modal content footer](/docs/5.x/actions/modals#adding-custom-modal-content-below-the-form)
- `ActionsRenderHook::MODAL_SCHEMA_AFTER` - After the [modal schema](/docs/5.x/actions/modals#rendering-a-schema-in-a-modal)
- `ActionsRenderHook::MODAL_SCHEMA_BEFORE` - Before the [modal schema](/docs/5.x/actions/modals#rendering-a-schema-in-a-modal)

### [​](#widgets-render-hooks)Widgets render hooks

```
use Filament\Widgets\View\WidgetsRenderHook;
```

- `WidgetsRenderHook::TABLE_WIDGET_END` - End of the [table widget](/docs/5.x/widgets/overview#table-widgets), after the table itself, also [can be scoped](#scoping-render-hooks) to the table widget class
- `WidgetsRenderHook::TABLE_WIDGET_START` - Start of the [table widget](/docs/5.x/widgets/overview#table-widgets), before the table itself, also [can be scoped](#scoping-render-hooks) to the table widget class

## [​](#scoping-render-hooks)Scoping render hooks
...
### [​](#retrieving-the-currently-active-scopes-inside-the-render-hook)Retrieving the currently active scopes inside the render hook
...
## [​](#passing-data-to-render-hooks)Passing data to render hooks
...
[Icons](/docs/5.x/styling/icons)[Registering assets](/docs/5.x/advanced/assets)
