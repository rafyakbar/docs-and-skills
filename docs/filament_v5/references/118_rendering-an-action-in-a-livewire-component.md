...
For example, if your action uses [Notifications](/docs/5.x/components/notifications), remember to include `@livewire('notifications')` in your layout and add `@import '../../vendor/filament/notifications/resources/css/index.css'` to your CSS file.If you are using any other [Filament components](/docs/5.x/components/overview#package-components) in your action, make sure to install and integrate the corresponding package as well.
...
You may [group actions together into a dropdown menu](/docs/5.x/actions/grouping-actions) by using the `<x-filament-actions::group>` Blade component, passing in the `actions` array as an attribute:
