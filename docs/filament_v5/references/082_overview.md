...
Optionally, a notification can have an [icon](/docs/5.x/styling/icons) that’s displayed in front of its content. You may also set a color for the icon, which is gray by default:
...
Notifications often have a status like `success`, `warning`, `danger` or `info`. Instead of manually setting the corresponding [icons](/docs/5.x/styling/icons) and [colors](/docs/5.x/styling/colors), there’s a `status()` method which you can pass the status. You may also use the dedicated `success()`, `warning()`, `danger()` and `info()` methods instead. So, cleaning up the above example would look like this:
...
Notifications support [Actions](/docs/5.x/actions/overview), which are buttons that render below the content of the notification. They can open a URL or dispatch a Livewire event. Actions can be defined as follows:
...
You can learn more about how to style action buttons [here](/docs/5.x/actions/overview).
